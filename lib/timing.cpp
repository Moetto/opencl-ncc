...
string diff(timeval t1, timeval t2) {
    ...
    long long diff;
    int million = 1000000;
    diff = t2.tv_sec * million - t1.tv_sec * million + t2.tv_usec - t1.tv_usec;
    secs = diff / million;
...
void Timer::outputDiff(timeStamp t1, timeStamp t2, timeStamp start) {
    std::cout << std::left << std::setfill('0') << std::setw(8) << diff(t1.time, t2.time) << "s ";
    std::cout << std::left << std::setfill('0') << std::setw(8) << diff(start.time, t2.time) << "s";
